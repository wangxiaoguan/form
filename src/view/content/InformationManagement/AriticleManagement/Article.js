...
@connect(
    state => ({
      ...
      pageDatadataSource:state.pageData,
      powers: state.powers,
      allData:state,
...
class CategoryAndList extends Component{
  ...
  componentDidMount(){
    this.props.setTimePushData('修改hi受到广泛vhi是德国')
...
class Article extends Component {
  ...
  componentDidMount(){
    ...
    console.log(JQuery.jQuery('#article').css({color:'red',fontSize:'70px'}))
  }

  //Tabs标签切换
  handleChangeTabs=(activeKey)=>{
    this.setState({currentTabsKey:activeKey});
    this.props.getData('111111111')
    if(activeKey=='1'){
  ...
  render() {
    ...
    const option = [{ key: '', value: '全部' },{ key: 2, value: '是' }, { key: 1, value: '否' }];
    const search = [
      { key: 'categoryId',      label: '所属栏目',    qFilter: 'Q=categoryid_S_LK', type:'cascader',option: []},
      { key: 'type',            label: '文章类型',    qFilter: 'Q=type_S_EQ',       type:'select',  option: option},
